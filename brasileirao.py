...
def nomes_dos_times_de_um_jogo(dados,id_jogo):
    ...
    id_t1 = jogo['time1']
    id_t2 = jogo['time2']
    nomet1= dados['equipes'][id_t1]['nome-comum']
    nomet2= dados['equipes'][id_t2]['nome-comum']
...
def datas_de_jogo(dados):
    list_datas=[]
    for dia in dados['fases']['2700']['jogos']['data']:
        list_datas.append(dia)
    return list_datas
...
def data_de_um_jogo(dados,id_jogo):
    dici_dias= dados['fases']['2700']['jogos']['data']
    for dia in dici_dias:
        for jogo in dici_dias[dia]:
            if jogo==id_jogo:
                return dia
    return 'nao encontrado'
...
def dicionario_id_estadio_e_nro_jogos(dados):
    ...
    for id in jogos:
        id_estadio=jogos[id]['estadio_id']
        if id_estadio not in estadios.keys():
...
def busca_imprecisa_por_nome_de_time(dados,nome_time):
    ...
    for id in equipes:
        if nome_time in equipes[id]['nome']:
            list_ids.append(id)
        elif nome_time in equipes[id]['nome-comum']:
            list_ids.append(id)
        elif nome_time in equipes[id]['nome-slug']:
            list_ids.append(id)
...
def ids_de_jogos_de_um_time(dados,time_id):
    ...
    jogos=dados['fases']['2700']['jogos']['id']
    for id in jogos:
        if time_id in jogos[id]['time1']:
            list_jogos.append(id)
        elif time_id in jogos[id]['time2']:
            list_jogos.append(id)
    return list_jogos
...
def datas_de_jogos_de_um_time(dados,nome_time):
    ...
    for id_jogo in jogos:
        if time_id in jogos[id_jogo]['time1']:
            list_datas.append(jogos[id_jogo]['data'])
        elif time_id in jogos[id_jogo]['time2']:
            list_datas.append(jogos[id_jogo]['data'])
...
def dicionario_de_gols(dados):
    gols={}
    jogos=dados['fases']['2700']['jogos']['id']
    for id_jogos in jogos:

        t1=jogos[id_jogos]['time1']
        t2=jogos[id_jogos]['time2']

        if t1 not in gols.keys():
            gols[t1]=int(jogos[id_jogos]['placar1'])
        else:
            gols[t1]+=int(jogos[id_jogos]['placar1'])
            
        if t2 not in gols.keys():
